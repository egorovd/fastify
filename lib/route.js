...
function buildRouting (options) {
  ...
  function route (opts) {
    ...
    if (opts.dynamicHack) {
      // console.log('path XXXXXXXXXXXXXXXX', opts)
      // return
      ...
      if (path === '/' && prefix.length > 0) {
        switch (opts.prefixTrailingSlash) {
          case 'slash':
            afterRouteAdded.call(this, path, null, () => {}
          )
            break
          case 'no-slash':
            afterRouteAdded.call(this, '', null, () => {}
          )
            break
          case 'both':
          default:
        ...
        afterRouteAdded.call(this, path.slice(1), null, () => {}
      )
      } else {
        afterRouteAdded.call(this, path, null, () => {})
      }
  ...
  function routeOff (opts) {
    // console.log('route OFF', opts)
    router.off(opts.method, opts.url)
    for (const hook of this[kGlobalHooks].offRoute) {
      try {
        hook.call(this, opts)
        return true
      } catch (error) {
